...
> [!WARNING]
> Мы используем только TRANSLATION DeepL
>
> Это может повлиять на качество перевода. Это также может привести к тому, что система неправильно идентифицирует данные вашего файла.

Это действие GitHub автоматически генерирует и отправляет локализованные версии ваших файлов *.md на основе поддерживаемых языков.
...
Замените `LANGS` на разделенный запятыми список языков, которые вы хотите сгенерировать.
<details>
<summary>Доступные языки для перевода (полный ссылки)</summary>
...
<details>
<summary>Доступные языки для перевода (краткий адреса)</summary>
...
- `FILES`: разделенный запятыми список файлов для перевода.
- `LANGS`: разделенный запятыми список языков для генерации.

## Пример

Например, если вы хотите сгенерировать файлы для сербского, итальянского и английского языков, ваша конфигурация будет выглядеть так:
...
И вы можете посмотреть, как работает действие, нажав на эти виджеты:
<div align="center">
